...
func (a *Analyzer) AnalyzeStatefulSet(r common.Request) (string, error) {
	kind := "StatefulSet"
	...
	stsList, err := a.k8s.Clientset.AppsV1().StatefulSets(r.Namespace).List(r.Context, metav1.ListOptions{})
	if err != nil {
		return "", err
	}
	...
	for _, sts := range stsList.Items {
		...
		if svcName != "" {
			_, err := a.k8s.Clientset.CoreV1().Services(sts.Namespace).Get(r.Context, svcName, metav1.GetOptions{})
			if err != nil {
		...
		if len(sts.Spec.VolumeClaimTemplates) > 0 {
			for _, volumeClaimTemplate := range sts.Spec.VolumeClaimTemplates {
				_, err := a.k8s.Clientset.CoreV1().PersistentVolumeClaims(sts.Namespace).Get(r.Context, volumeClaimTemplate.Name, metav1.GetOptions{})
				if err != nil {
		...
		if sts.Spec.Replicas != nil && *(sts.Spec.Replicas) != sts.Status.AvailableReplicas {
			for i := int32(0); i < *(sts.Spec.Replicas); i++ {
				...
				pod, err := a.k8s.Clientset.CoreV1().Pods(sts.Namespace).Get(r.Context, podName, metav1.GetOptions{})
				if err != nil {
					if errors.IsNotFound(err) && i == 0 {
						evt, err := utils.FetchLatestEvent(a.k8s.Clientset, sts.Namespace, sts.Name)
	...
	for key, value := range preAnalysis {
		...
		parent, found := utils.GetParent(a.k8s.Clientset, value.StatefulSet.ObjectMeta)
		if found {
