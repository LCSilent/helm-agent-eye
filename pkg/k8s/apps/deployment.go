...
func (a *Analyzer) DeploymentScale(ctx context.Context, namespace, name string, replicas int32) (string, error) {
	deploy, err := a.k8s.Clientset.AppsV1().Deployments(namespace).Get(ctx, name, metav1.GetOptions{})
	if err != nil {
		return "", err
	}

	deploy.Spec.Replicas = &replicas
	_, err = a.k8s.Clientset.AppsV1().Deployments(namespace).Update(ctx, deploy, metav1.UpdateOptions{})
	if err != nil {
		return "", err
	}
...
// AnalyzeDeployment analyzes the deployments and returns a list of failures.
func (a *Analyzer) AnalyzeDeployment(ctx context.Context, namespace string) (string, error) {
	kind := "Deployment"
	...
	deployList, err := a.k8s.Clientset.AppsV1().Deployments(namespace).List(ctx, metav1.ListOptions{})
	if err != nil {
		return "", err
	}
