...
func (a *Analyzer) AnalyzeNode(ctx context.Context, name string) (string, error) {
	kind := "Node"
	nodes := make([]v1.Node, 0)
	if name == "" {
		nodeList, err := a.k8s.Clientset.CoreV1().Nodes().List(ctx, metav1.ListOptions{})
		if err != nil {
			return "", err
		}
		nodes = nodeList.Items
	} else {
		node, err := a.k8s.Clientset.CoreV1().Nodes().Get(ctx, name, metav1.GetOptions{})
		if err != nil {
			return "", err
		}
	...
	for key, value := range preAnalysis {
		...
		parent, found := utils.GetParent(a.k8s.Clientset, value.Node.ObjectMeta)
		if found {
