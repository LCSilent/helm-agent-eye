...
func (a *Analyzer) AnalyzeIngress(r common.Request) (string, error) {
	kind := "Ingress"
	...
	ingresses, err := a.k8s.Clientset.NetworkingV1().Ingresses(r.Namespace).List(r.Context, metav1.ListOptions{
	...
	for _, ingress := range ingresses.Items {
		...
		if ingressClassName != nil {
			_, err := a.k8s.Clientset.NetworkingV1().IngressClasses().Get(r.Context, *ingressClassName, metav1.GetOptions{})
			if err != nil {
				doc := apiDoc.GetApiDocV2("spec.ingressClassName")
				failures = append(failures, common.Failure{
		...
		for _, rule := range ingress.Spec.Rules {
			...
			for _, path := range rule.HTTP.Paths {
				_, err := a.k8s.Clientset.CoreV1().Services(ingress.Namespace).Get(r.Context, path.Backend.Service.Name, metav1.GetOptions{})
				if err != nil {
		...
		for _, tls := range ingress.Spec.TLS {
			_, err := a.k8s.Clientset.CoreV1().Secrets(ingress.Namespace).Get(r.Context, tls.SecretName, metav1.GetOptions{})
			if err != nil {
	...
	for key, value := range preAnalysis {
		...
		parent, found := utils.GetParent(a.k8s.Clientset, value.Ingress.ObjectMeta)
		if found {
