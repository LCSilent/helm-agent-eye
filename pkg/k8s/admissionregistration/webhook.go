...
func (a *Analyzer) AnalyzeValidatingWebhook(r common.Request) (string, error) {
	...
	webhookList, err := a.k8s.Clientset.AdmissionregistrationV1().ValidatingWebhookConfigurations().List(r.Context, metav1.ListOptions{
	...
	for _, webhookConfig := range webhookList.Items {
		...
		for i, wh := range webhookConfig.Webhooks {
			if wh.ClientConfig.Service != nil {
				svc, err := a.k8s.Clientset.CoreV1().Services(wh.ClientConfig.Service.Namespace).Get(
				...
				if err != nil {
					...
					podList, err := a.k8s.Clientset.CoreV1().Pods(wh.ClientConfig.Service.Namespace).List(r.Context, metav1.ListOptions{
...
func (a *Analyzer) AnalyzeMutatingWebhook(r common.Request) (string, error) {
	...
	webhookList, err := a.k8s.Clientset.AdmissionregistrationV1().MutatingWebhookConfigurations().List(r.Context, metav1.ListOptions{
		LabelSelector: r.LabelSelector,
	})
	if err != nil {
		return "", err
